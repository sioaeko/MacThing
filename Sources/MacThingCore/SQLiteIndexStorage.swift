...
private final class SQLiteDatabase {
    ...
    private let derivedColumnsBackfillKey = "derivedColumnsBackfilledV5"
    ...
    func createSchema() throws {
        ...
        try execute("""
            CREATE TABLE IF NOT EXISTS entries (
                ...
                exact_name_part TEXT NOT NULL DEFAULT '',
                exact_path_part TEXT NOT NULL DEFAULT '',
        ...
        try? execute("ALTER TABLE entries ADD COLUMN exact_name_part TEXT NOT NULL DEFAULT '';")
        try? execute("ALTER TABLE entries ADD COLUMN exact_path_part TEXT NOT NULL DEFAULT '';")
        ...
        try execute("CREATE INDEX IF NOT EXISTS entries_exact_name_part_idx ON entries(exact_name_part);")
        try execute("CREATE INDEX IF NOT EXISTS entries_exact_path_part_idx ON entries(exact_path_part);")
    ...
    private func derivedLengthValues(path: String, name: String, parent: String) -> DerivedLengthValues {
        ...
        return DerivedLengthValues(
            ...
            exactNamePart: exactNamePart,
            exactPathPart: exactPathPart,
            nameLength: name.utf16.count,
            namePartLength: exactNamePart.utf16.count,
            nameCharacterCount: name.count,
            pathLength: path.utf16.count,
            pathPartLength: exactPathPart.utf16.count,
            ...
            try withStatement("""
                ...
                for row in rows {
                    ...
                    try statement.bind([
                        ...
                        .text(derived.exactNamePart),
                        .text(derived.exactPathPart),
    ...
    private func candidateFilter(for hint: SearchCandidateHint, tablePrefix: String?) -> SQLiteCandidateFilter {
        ...
        for filter in hint.fileListSourceFilters {
            let sourceFilter = fileListSourceFilterClause(prefix: prefix, filter: filter)
            clauses.append(sourceFilter.clause)
        ...
        for filter in hint.childRunCountFilters {
            let childFilter = childRunCountFilterClause(
                prefix: prefix,
                tablePrefix: tablePrefix,
                filter: filter
            )
        ...
        for filter in hint.childDateFilters {
            let childFilter = childDateFilterClause(
                prefix: prefix,
                tablePrefix: tablePrefix,
                filter: filter
            )
    ...
    private func fileListSourceFilterClause(
        prefix: String,
        filter: SearchCandidateFileListSourceFilter
    ) -> (clause: String, bindings: [SQLiteValue]) {
        var matchClauses: [String] = []
        var bindings: [SQLiteValue] = []
        ...
        if !filter.nameValues.isEmpty {
            let values = filter.nameValues.sorted()
            matchClauses.append("\(prefix)file_list_name IN (\(placeholders(count: values.count)))")
            bindings.append(contentsOf: values.map(SQLiteValue.text))

            for value in values {
                matchClauses.append("(\(prefix)file_list_path = ? OR \(prefix)file_list_path LIKE ? ESCAPE '\\')")
        ...
        if !filter.pathValues.isEmpty {
            let values = filter.pathValues.sorted()
            matchClauses.append("\(prefix)file_list_path IN (\(placeholders(count: values.count)))")
            bindings.append(contentsOf: values.map(SQLiteValue.text))
        }

        guard !matchClauses.isEmpty else {
            return ("(\(prefix)file_list_name IS NOT NULL AND \(prefix)file_list_path IS NOT NULL)", [])
        }
    ...
    private func numericExpression(for field: SearchCandidateNumericField, prefix: String, tablePrefix: String?) -> String {
        ...
        case .namePartFrequency:
            return valueFrequencyExpression(
                tablePrefix: tablePrefix,
                column: "exact_name_part",
                alias: "same_name_part",
                allowNullMatch: true
            )
        ...
        case .pathPartFrequency:
            return valueFrequencyExpression(
                tablePrefix: tablePrefix,
                column: "exact_path_part",
                alias: "same_path_part",
                allowNullMatch: true
            )
    ...
    private func childRunCountFilterClause(
        prefix: String,
        tablePrefix: String?,
        filter: SearchCandidateChildRunCountFilter
    ) -> (clause: String, bindings: [SQLiteValue]) {
        let pathExpression = outerPathExpression(tablePrefix: tablePrefix)
        ...
        for numericFilter in filter.filters {
            childClauses.append("child.run_count \(sqlOperator(for: numericFilter.op)) ?")
            bindings.append(.int(numericFilter.value))
        }
    ...
    private func childDateFilterClause(
        prefix: String,
        tablePrefix: String?,
        filter: SearchCandidateChildDateFilter
    ) -> (clause: String, bindings: [SQLiteValue]) {
        let pathExpression = outerPathExpression(tablePrefix: tablePrefix)
        var childClauses = ["child.parent = \(pathExpression)"]
        var bindings: [SQLiteValue] = []
    ...
    private func insert(entries: [FileEntry]) throws {
        let entrySQL = """
            INSERT OR REPLACE INTO entries (
                ...
                exact_path_part, name_length, name_part_length, name_character_count,
                path_length, path_part_length, extension_length, name_utf8_length,
                path_utf8_length, kind, byte_size, created_at, modified_at, accessed_at,
                indexed_at, run_count, last_run_at, attributes, file_id, volume_id,
                file_list_name, file_list_path, media_title, media_artist, media_album,
                media_comment, media_genre, media_track, media_year
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
            """
