...
- SQLite candidate seeding also applies supported structured filters such as
  ...
  (`filelistfilename:`) filters, exact existence filters, exact path-list
  filters, empty-entry (`empty:`) filters, exact direct-parent filters,
  direct-child count, child attr/date/run-count/size,
