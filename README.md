...
- SQLite candidate seeding also applies supported structured filters such as
  extension, semicolon extension/category lists, kind/category, file size including `size:unknown`, dates including
  `dm:unknown`, file attributes, media track/year metadata, filesystem identity/value-list
  ...
  parent-sibling, ancestor-sibling, and sibling presence/exact filters, parent
  ...
  filters, exact name/extension frequency filters, exact name/stem/path-part
  duplicate filters, size/date/attribute duplicate filters, and
...
- Settings > Shortcuts provides an Everything-style key mapping table: click a
  ...
  quick choices and disabling. Each row shows its default shortcut, can be reset
